                ...
                <Transition name="slide-up">
                    ...
                    >
                        ...
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <button @click="$emit('close')" style="color: hsl(var(--muted-foreground))">
                                <X :size="20" />
                            </button>
                        </div>
                        ...
                        <div v-if="!isGenerated" class="space-y-4">
                            ...
                            <p v-if="error" class="text-xs" style="color: hsl(var(--destructive))">
                                {{ error }}
                            </p>
                        ...
                        <div v-else>
                            ...
                            >
                                <Loader v-if="isDownloading" :size="16" class="animate-spin" />
...
<script setup lang="ts">
    ...
    const validate = () => {
        if (!form.contactPerson || !form.companyName || !form.companyAddress || !form.jobTitle || !form.technologies) {
            error.value = form.language === 'german' ?
    ...
    const getTechSentence = () => {
        ...
        const hasPhp = techs.some(t => t.toLowerCase().includes('php') || t.toLowerCase().includes('laravel'))
        const otherTechs = techs.filter(t =>
    ...
    const downloadMergedPdf = async () => {
        ...
        try {
            ...
            // Subject line
            const subject = isGerman ? `Bewerbung als ${form.jobTitle}` : `Application for ${form.jobTitle}`
            ...
            // Salutation
            const salutation = isGerman ? `Sehr geehrte/r ${form.contactPerson},` : `Dear ${form.contactPerson},`
            ...
            y = drawWrappedText(page1, p1, margin, y, contentWidth, 10, font, black, 16)
            y -= 12
            ...
            y = drawWrappedText(page1, p2, margin, y, contentWidth, 10, font, black, 16)
            y -= 12

            // Paragraph 3 - tech
            if (techSentence) {
                y = drawWrappedText(page1, techSentence.trim(), margin, y, contentWidth, 10, font, black, 16)
                y -= 12
            }
            ...
            y = drawWrappedText(page1, p4, margin, y, contentWidth, 10, font, black, 16)
            y -= 30
            ...
            console.error(e)
            error.value = 'Something went wrong generating the PDF. Please try again.'
        } finally {
